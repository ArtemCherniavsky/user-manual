# setup-docs.ps1
# Sets up a Jekyll docs project in a new folder.
#
# Run:
#   .\setup-docs.ps1
#
...
function Ask([string]$q, [string]$d = "") {
    $hint = if ($d) { " [$d]" } else { "" }
    $a = Read-Host "$q$hint"
...
function WriteFile([string]$filePath, [string]$text) {
    $dir = Split-Path $filePath
    if ($dir -and !(Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    [System.IO.File]::WriteAllText($filePath, $text, $enc)
    $rel = $filePath.Replace((Get-Location).Path + "\", "")
    Write-Host "  created: $rel" -ForegroundColor Green
}

Write-Host ""
Write-Host "=== Jekyll Docs Setup ===" -ForegroundColor Cyan
Write-Host ""

$targetDir  = Ask "Project folder (will be created)" "my-docs"
$title      = Ask "Documentation title" "My Documentation"
$repoName   = Ask "GitHub repository name" $targetDir
$ghUser     = Ask "GitHub username" "username"
$logoUrl    = Ask "Logo URL (SVG/PNG, Enter to skip)" ""
$link1Label = Ask "Link 1 label" "Video"
$link1Url   = Ask "Link 1 URL" ""
$link2Label = Ask "Link 2 label" "Samples"
$link2Url   = Ask "Link 2 URL" ""
$link3Label = Ask "Link 3 label" "Support"
$link3Url   = Ask "Link 3 URL" ""

$root = Join-Path (Get-Location).Path $targetDir
Write-Host ""
Write-Host "Creating files in: $root" -ForegroundColor Yellow
Write-Host ""

# Logo and nav links
if ($logoUrl) {
    $logoHtml = "        <img src=`"$logoUrl`" alt=`"$title`">"
} else {
    $logoHtml = "        <span style=`"font-weight:600;font-size:15px;color:var(--color-text)`">$title</span>"
}

$navLines = @()
if ($link1Url) { $navLines += "        <a href=`"$link1Url`" target=`"_blank`" rel=`"noopener`">$link1Label</a>" }
if ($link2Url) { $navLines += "        <a href=`"$link2Url`" target=`"_blank`" rel=`"noopener`">$link2Label</a>" }
if ($link3Url) { $navLines += "        <a href=`"$link3Url`" target=`"_blank`" rel=`"noopener`">$link3Label</a>" }
$navLinks = $navLines -join "`n"

# ── _config.yml ──────────────────────────────────────────────────────────────
WriteFile (Join-Path $root "_config.yml") @"
title: $title
...
WriteFile (Join-Path $root "_includes\sidebar_item.html") @'
...
WriteFile (Join-Path $root "_includes\breadcrumbs.html") @'
{% assign current_url = page.url | remove: ".html" | remove_trailing_slash | prepend: site.baseurl %}
{% assign crumbs = "" %}
...
WriteFile (Join-Path $root "scripts\generate_sidebar.js") @'
#!/usr/bin/env node
const fs = require("fs");
...
const ITEM_RE = /^( *)- \[([^\]]+)\]\(([^)]+)\)/;
const EXCLUDE_PATHS = []; // Add folders to exclude, e.g.: ["Out_Of_Date/"]
...
WriteFile (Join-Path $root ".github\workflows\deploy.yml") @'
...
# Single-quoted here-string: Liquid/JS syntax is not interpreted by PowerShell
# Placeholders XLOGOX and XNAVX are replaced after
$defaultHtml = @'
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #header-search { width: 100%; max-width: 700px; position: relative; justify-self: center; }

    #search-input {
      width: 100%; padding: 7px 10px 7px 32px;
      border: 1px solid var(--color-border); border-radius: 6px;
      background: var(--color-sidebar-bg); font-family: inherit; font-size: 13px;
      color: var(--color-text); outline: none;
      transition: border-color 0.15s, box-shadow 0.15s, background 0.15s;
    }
    #search-input:focus { border-color: var(--color-accent); box-shadow: 0 0 0 3px rgba(51,138,219,0.1); background: var(--color-bg); }
    ...
    .search-result-title { font-size: 13px; font-weight: 500; margin-bottom: 2px; }
    ...
    #sidebar-nav { flex: 1; overflow-y: auto; padding: 16px 8px 24px; scrollbar-width: thin; scrollbar-color: #d1d5db transparent; scrollbar-gutter: stable; }
    ...
    #sidebar-nav a { display: flex; align-items: center; gap: 6px; padding: 5px 10px; border-radius: 6px; color: var(--color-text-sidebar); text-decoration: none; font-size: 13.5px; line-height: 1.4; transition: background 0.1s, color 0.1s; }
    ...
    #sidebar-nav li.has-children > a::after {
      content: ""; display: inline-block; margin-left: auto; width: 14px; height: 14px;
      ...
      background-repeat: no-repeat; background-position: center; flex-shrink: 0; transition: transform 0.15s;
    }
...
# Inject logo and nav links
$defaultHtml = $defaultHtml.Replace('XLOGOX', $logoHtml).Replace('XNAVX', $navLinks)
WriteFile (Join-Path $root "_layouts\default.html") $defaultHtml

# ── index.md + sample pages (only if not present) ────────────────────────────
if (!(Test-Path (Join-Path $root "index.md"))) {
    WriteFile (Join-Path $root "index.md") @"
# $title

- [Getting Started](docs/getting-started.md)
- [Installation](docs/installation.md)
"@
    WriteFile (Join-Path $root "docs\getting-started.md") @"
# Getting Started

Welcome to the documentation!
"@
    WriteFile (Join-Path $root "docs\installation.md") @"
# Installation
...
# ── .gitignore ────────────────────────────────────────────────────────────────
WriteFile (Join-Path $root ".gitignore") @'
_site/
_data/sidebar.yml
.jekyll-cache/
.sass-cache/
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "Done! Project created in: $root" -ForegroundColor Cyan
Write-Host ""
Write-Host "Next steps:" -ForegroundColor Yellow
Write-Host "  1. Put your .md files into the project folder"
Write-Host "  2. Update index.md with links to your files"
Write-Host "  3. Create repository: https://github.com/new (name: $repoName)"
Write-Host "  4. Run:"
...
Write-Host "  5. Settings -> Pages -> Source -> GitHub Actions"
Write-Host "  6. Your site: https://$($ghUser).github.io/$repoName/"
Write-Host ""
